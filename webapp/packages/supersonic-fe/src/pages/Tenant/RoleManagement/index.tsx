...
const TenantRoleManagement: React.FC = () => {
  ...
  const loadPermissions = async () => {
    try {
      const { code, data } = await getTenantPermissions();
      if (code === 200 && data) {
        setPermissions(data);
      }
    } catch (error) {
  ...
  const handleDelete = async (id: number) => {
    try {
      const { code } = await deleteTenantRole(id);
      if (code === 200) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const data = { ...values, scope: 'TENANT' };
      let result;
      if (editingRole) {
        result = await updateTenantRole(editingRole.id, data);
      } else {
        result = await createTenantRole(data);
      }
  ...
  const handleSavePermissions = async () => {
    ...
    try {
      setLoading(true);
      const { code } = await assignPermissionsToTenantRole(editingRole.id, selectedPermKeys.map(Number));
      if (code === 200) {
    ...
    <div className={styles.container}>
      ...
      >
        <Form form={form} layout="vertical" initialValues={{ status: true }}>
          ...
          >
            <Input placeholder="如: TENANT_ADMIN, ANALYST, VIEWER" disabled={!!editingRole} />
          </Form.Item>
          <Form.Item
            name="name"
            label="角色名称"
          ...
          >
            <Input placeholder="如: 租户管理员, 分析师, 查看者" />
          </Form.Item>
          <Form.Item label="作用域">
            <Select value="TENANT" disabled>
              <Select.Option value="TENANT">租户级</Select.Option>
            </Select>
          </Form.Item>
