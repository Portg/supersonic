...
const TenantPermissionManagement: React.FC = () => {
  ...
  const handleAdd = () => {
    setEditingPerm(null);
    form.resetFields();
    form.setFieldsValue({ type: 'MENU', scope: 'TENANT' });
  ...
  const handleDelete = async (id: number) => {
    try {
      const { code } = await deleteTenantPermission(id);
      if (code === 200) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      let result;
      if (editingPerm) {
        result = await updateTenantPermission(editingPerm.id, data);
      } else {
        result = await createTenantPermission(data);
      }
    ...
    <div className={styles.container}>
      ...
      >
        <Form form={form} layout="vertical" name="tenantPermissionForm">
          ...
          >
            <Input placeholder="如: TENANT_ORG_MANAGE" disabled={!!editingPerm} />
          </Form.Item>
          <Form.Item
            name="name"
            label="权限名称"
