...
import {
  queryDataSetAuthGroups,
  removeDataSetAuthGroup,
} from '@/services/datasetAuth';
import { getOrganizationTree } from '@/components/SelectPartner/service';
import DatasetPermissionDrawer from './DatasetPermissionDrawer';
...
const DatasetPermissionTable: React.FC<Props> = ({
  datasetId,
  dimensionList = [],
  metricList = [],
}) => {
  ...
  const queryListData = async () => {
    ...
    const { code, data } = await queryDataSetAuthGroups(datasetId);
    if (code === 200) {
      setIntentionList(data || []);
      return;
    }
    message.error('获取数据集权限组失败');
  };
  ...
  const columns: ProColumns[] = [
    ...
    },
    {
      ...
      render: (_, record: any) => {
        ...
        const personNameList = tmePerson.reduce((enNames: string[], item: any) => {
          ...
          if (hasPerson) {
            enNames.push(item.displayName || item.name);
          }
          return enNames;
        }, []);
    ...
    {
      dataIndex: 'inheritFromModel',
      title: '继承模型权限',
      width: 100,
      render: (_, record: any) => {
        return record.inheritFromModel === 1 ? '是' : '否';
      },
    },
    {
      title: '操作',
        ...
        return (
          <Space>
            ...
            <Popconfirm
              ...
              cancelText="否"
              onConfirm={async () => {
                const { code, msg } = await removeDataSetAuthGroup(record.groupId);
                if (code === 200) {
                  setPermissonData({});
                  queryListData();
                } else {
            ...
            >
              <a key="permissionDeleteBtn">删除</a>
            </Popconfirm>
          </Space>
        );
      },
...
export default DatasetPermissionTable;
