...
const PlatformRoleManagement: React.FC = () => {
  ...
  const loadRoles = async () => {
    setLoading(true);
    try {
      const { code, data } = await getPlatformRoles();
      if (code === 200 && data) {
        setRoles(data);
      }
    } catch (error) {
  ...
  const loadPermissions = async () => {
    try {
      const { code, data } = await getPlatformPermissions();
      if (code === 200 && data) {
        setPermissions(data);
      }
    } catch (error) {
  ...
  const handleDelete = async (id: number) => {
    try {
      const { code } = await deletePlatformRole(id);
      if (code === 200) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const data = { ...values, scope: 'PLATFORM' };
      let result;
      if (editingRole) {
        result = await updatePlatformRole(editingRole.id, data);
      } else {
        result = await createPlatformRole(data);
      }
  ...
  const handleConfigPermissions = async (role: Role) => {
    ...
    setLoading(true);
    try {
      const { code, data } = await getRolePermissionIds(role.id);
      if (code === 200 && data) {
  ...
  const handleSavePermissions = async () => {
    ...
    try {
      setLoading(true);
      const { code } = await assignPermissionsToRole(editingRole.id, selectedPermKeys.map(Number));
      if (code === 200) {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'scope',
      key: 'scope',
      render: () => <Tag color="purple">平台级</Tag>,
    },
    {
      title: '类型',
      dataIndex: 'isSystem',
  ...
  return (
    <div className={styles.container}>
      ...
      <Modal
        ...
        cancelText="取消"
      >
        <Form form={form} layout="vertical" name="platformRoleForm" initialValues={{ status: true }}>
          <Form.Item
            name="code"
            label="角色编码"
            rules={[{ required: true, message: '请输入角色编码' }]}
          >
            <Input placeholder="如: PLATFORM_ADMIN, PLATFORM_OPERATOR" disabled={!!editingRole} />
          </Form.Item>
          <Form.Item
            name="name"
            label="角色名称"
          ...
          >
            <Input placeholder="如: 平台管理员, 平台运营" />
          </Form.Item>
          <Form.Item label="作用域">
