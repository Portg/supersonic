...
const PlatformPermissionManagement: React.FC = () => {
  ...
  const handleAdd = () => {
    setEditingPerm(null);
    form.resetFields();
    form.setFieldsValue({ type: 'MENU', scope: 'PLATFORM' });
  ...
  const handleDelete = async (id: number) => {
    try {
      const { code } = await deletePlatformPermission(id);
      if (code === 200) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      let result;
      if (editingPerm) {
        result = await updatePlatformPermission(editingPerm.id, data);
      } else {
        result = await createPlatformPermission(data);
      }
  ...
  return (
    <div className={styles.container}>
      ...
      <Modal
        ...
        cancelText="取消"
      >
        <Form form={form} layout="vertical" name="platformPermissionForm">
          <Form.Item
            name="code"
            label="权限编码"
            rules={[{ required: true, message: '请输入权限编码' }]}
          >
            <Input placeholder="如: PLATFORM_TENANT_MANAGE" disabled={!!editingPerm} />
          </Form.Item>
          <Form.Item
            name="name"
            label="权限名称"
